...
template __global__ void gemm_a16w16_mono_tile_kernel<
    opus_gemm_traits<512, 192, 256, 64, bf16_t, bf16_t, bf16_t, float, 8, 8, 4>>(opus_gemm_kargs);
#else
#include "gemm_a16w16_mono_tile_kernel_template.hpp"
template __global__ void gemm_a16w16_mono_tile_kernel<
    opus_gemm_traits<512, 192, 256, 64, bf16_t, bf16_t, bf16_t, float, 8, 8, 4>>(opus_gemm_kargs);
#endif
